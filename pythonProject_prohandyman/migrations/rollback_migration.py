...
# Если в SQL не найден ни один database path — используем этот (можешь подставить свой)
...
def detect_all_db_paths_from_sql(sql_text: str) -> List[str]:
    """
    Собираем все database path, которые встречаются в файле миграции.
    Ищем по абсолютным путям в бэктиках (устойчиво к многострочным DDL).
    """
    paths: List[str] = []
    for m in re.finditer(ABS_PATH_IN_BACKTICKS, sql_text):
...
def main() -> None:
    ...
    # определим ВСЕ database path из файла (если есть)
    db_paths = detect_all_db_paths_from_sql(raw_sql)
    if db_paths:
        print(f"📦 Файл содержит database path: {len(db_paths)}")
        for t in db_paths[:12]:
            print(f"   • {t}")
        if len(db_paths) > 12:
            print(f"   … и ещё {len(db_paths)-12}")
    else:
        print("📦 В SQL не обнаружены абсолютные пути — используем database path по умолчанию.")
        db_paths = [YDB_DATABASE_PATH_DEFAULT]

    print("🔗 Готовим версию и DSN...")
    ...
    if version is None:
        print(
            ...
            "Переименуй файл с числовым префиксом (например, 20251026163810_*.sql)."
        )
        sys.exit(2)
    ...
    # Проходимся по всем database path в файле (или по дефолтному)
    for dbp in db_paths:
        print(f"\n=== ⏪ Откат по базе: {dbp} (down-to {target}) ===")
        dsn = build_dsn(YDB_SECURE_ENDPOINT, dbp, token)

        print("🔎 Статус до отката:")
        ...
        if cur_ver is not None and cur_ver < version:
            print(f"> Выбранная миграция {version} ещё не применена (текущая версия {cur_ver}). Откатывать нечего.")
        else:
