...
class YdbCli:
    ...
    def whoami(self) -> RunResult:
        return run(self.base() + ["discovery", "whoami"], timeout=YDB_TIMEOUT, echo_stdout=True, raw_label="RAW_YDB_WHOAMI")
    ...
    def describe(self, abs_path: str) -> Optional[Dict[str, Any]]:
        """
        Устойчивый describe: делаем несколько попыток при сетевых сбоях.
        Если stdout непустой, парсим его даже при ненулевом коде возврата.
        Возвращаем словарь с ключами: _header, columns, primaryKey, _columns_raw.
        """
        attempts = 3
        last = None
        for i in range(attempts):
            rr = run(
                self.base() + ["scheme", "describe", abs_path],
                timeout=YDB_TIMEOUT,
                raw_label=f"RAW_YDB_DESCRIBE:{abs_path}",
            )
            last = rr
            # Если есть какой-то вывод — пробуем его парсить (даже при коде != 0)
            if (rr.out or "").strip():
                break

            # Транзиентные сетевые проблемы — попробуем ещё раз с небольшой паузой
            err = (rr.err or "")
            transient = (
                "UNAVAILABLE" in err
                or "too_many_pings" in err
                or "End of TCP stream" in err
                or "DeadlineExceeded" in err
            )
            if transient and i + 1 < attempts:
                time.sleep(0.5 * (i + 1))  # экспоненциальная маленькая пауза
                continue

            # Нет stdout и/или ошибка нетранзиентная — сдаёмся
            return None

        if not last or not (last.out or "").strip():
            return None
...
def main() -> None:
    ...
    if rr_who.code != 0:
        try:
            IAM_TOKEN_FILE.unlink(missing_ok=True)
        except Exception:
            pass
        ensure_iam_token()
        ydb = YdbCli(db.endpoint, db.database, IAM_TOKEN_FILE)
        ...
        if rr_who.code != 0:
            raise CmdError(
                "> Авторизация в YDB не удалась. Убедись, что выполнен 'yc init' и есть доступ к базе, либо используй сервисный ключ."
            )
