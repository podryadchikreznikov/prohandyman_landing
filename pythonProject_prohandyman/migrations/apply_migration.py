...
PATTERNS: List[Tuple[str, str, bool]] = [
    ...
    # Наш частный кейс: в файл под-миграции попадает неполный ALTER (без ADD/DROP ...)
    (r"no viable alternative at input 'ALTER TABLE'", "> В файл под-миграции попало неполное выражение: проверь сборку SQL до ';'.", False),
]
...
# ---------- парсинг SQL: полные выражения до ';' ----------
...
def group_sql_statements_by_dbpath(sql_text: str) -> Dict[str, List[str]]:
    """
    Группируем по database path ПОЛНЫЕ SQL-выражения (до ';').
    Как только встретили строку с абсолютным путём в бэктиках — начинаем буфер
    и копим все строки до ближайшей точки с запятой.
    """
    groups: Dict[str, List[str]] = {}
    cur_db: Optional[str] = None
    buf: List[str] = []

    # Пробегаем по строкам, но «граница выражения» — это символ ';'
    for raw in sql_text.splitlines():
        line = raw.rstrip()

        if cur_db is None:
            # Ищем старт выражения — строку с абсолютным путём в бэктиках
            m = ABS_PATH_IN_BACKTICKS.search(line)
            if not m:
                continue
            db_path = extract_db_path_from_abs_table(m.group(1))
            if not db_path:
                continue
            cur_db = db_path
            buf = [line]
        else:
            # Уже внутри выражения — просто копим строки
            buf.append(line)

        # Завершение выражения — встретили ';' где-либо в текущей строке
        if cur_db is not None and ";" in line:
            stmt = "\n".join(buf).strip()
            if stmt:
                groups.setdefault(cur_db, []).append(stmt)
            cur_db = None
            buf = []

    # Если файл закончился, а ';' не встретилась — зафиксируем буфер как есть
    if cur_db is not None and buf:
        stmt = "\n".join(buf).strip()
        if stmt:
            groups.setdefault(cur_db, []).append(stmt)
...
def main() -> None:
    ...
    # НОВОЕ: группируем по database path ПОЛНЫЕ выражения (до ';')
    up_groups = group_sql_statements_by_dbpath(up_text)
    down_groups = group_sql_statements_by_dbpath(down_text)

    if not up_groups:
        print(
            "⚠️  В секции Up не найдено ни одного выражения с полным путём `/ru-central1/...`.\n"
    ...
    for db_path in all_db_paths:
        ...
        subset_sql = build_subset_migration(
            up_stmts=up_groups.get(db_path, []),
            down_stmts=down_groups.get(db_path, []),
        )

        # немножко диагностического RAW — первые 500 символов
        preview = subset_sql[:500].replace("\n", "\\n")
        print(f"  === RAW_SUBSET_SQL_PREVIEW_BEGIN ===\n  {preview}\n  === RAW_SUBSET_SQL_PREVIEW_END ===")
