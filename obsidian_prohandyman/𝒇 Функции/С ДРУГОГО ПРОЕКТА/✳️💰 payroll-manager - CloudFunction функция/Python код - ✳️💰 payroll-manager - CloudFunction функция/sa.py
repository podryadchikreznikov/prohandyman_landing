...
def get_ydb_credentials(logger: JsonLogger):
    ...
    try:
        ...
        if "169.254.169.254" in msg or "computeMetadata" in msg:
            logger.error(
                "payroll_manager.lockbox_bootstrap_sa_missing",
                error=msg,
                hint="Cloud Function must have a service account attached to access Lockbox via SDK() metadata token.",
            )
        else:
            logger.error("payroll_manager.lockbox_failed", error=msg, trace=traceback.format_exc())
        raise
...
def get_iam_token(logger: JsonLogger) -> Optional[str]:
    ...
    try:
        ...
        if not token:
            logger.error("payroll_manager.iam_token.failed", error="Token value is empty")
            return None
        ...
        logger.info("payroll_manager.iam_token.refreshed", expires_in_sec=int(expires_at_ts - now_ts))
        return token
    except Exception as e:
        msg = str(e)
        if "169.254.169.254" in msg or "computeMetadata" in msg:
            logger.error(
                "payroll_manager.iam_token.bootstrap_sa_missing",
                error=msg,
                hint="Cloud Function must have a service account attached to access Lockbox via SDK() metadata token.",
            )
        else:
            logger.error("payroll_manager.iam_token.failed", error=msg, trace=traceback.format_exc())
        return None